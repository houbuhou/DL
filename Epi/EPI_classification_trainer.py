...
from unet.unet_model import DepthUNet, UNet, CNN
...
def eval_net_classification(net, dataset):
    net.eval()
    acc = 0
    for i in range(dataset.__len__()):
        img, true_mask, label = dataset[i]
        img = img.unsqueeze(0)
        img = img.cuda(device)
        prediction = net(img)
        # print('label:{}, predicted:{}'.format(label, prediction))
        prediction = prediction.data
        prediction, PredictedLocation = tensor_max(prediction)
        _, LabelLocation = tensor_max(label)
        if PredictedLocation == LabelLocation:
            acc += 1
...
def train_net(net,
    ...
    for epoch in tqdm(range(epochs)):
        ...
        # for img, true_mask in train_loader:
            ...
            loss = 0
            loss += criterion_BCE(mask_prob_flat, label.view(-1))
        ...
        val_dice_item = eval_net_classification(net, val)
